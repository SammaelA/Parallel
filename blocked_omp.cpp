...
void transfer(int blocks_x, int blocks_y, int blocks_z, int cur_grid, std::vector<std::array<Grid, 3>> &ring_buffers)
{
        ...
        for (int k=0;k<blocks_z;k++)
        {
          ...
          if (i != 0)
          {
            ...
            for (int y = 0; y < S.yn; y++)
              for (int z = 0; z < S.zn; z++)
                R.data[(R.xn-1)*R.yn*R.zn + y*R.zn + z] = S.data[(1)*S.yn*S.zn + y*S.zn + z];
          }
          if (i != blocks_x - 1)
          {
            Grid &R = ring_buffers[(i+1)*blocks_y*blocks_z + j*blocks_z + k][cur_grid];
            ...
            for (int y = 0; y < S.yn; y++)
              for (int z = 0; z < S.zn; z++)
                R.data[(0)*R.yn*R.zn + y*R.zn + z] = S.data[(S.xn - 2)*S.yn*S.zn + y*S.zn + z];          
          }
          ...
          if (j != 0)
          {
            ...
            for (int x = 0; x < S.xn; x++)
              for (int z = 0; z < S.zn; z++)
                R.data[x*R.yn*R.zn + (R.yn-1)*R.zn + z] = S.data[x*S.yn*S.zn + (1)*S.zn + z];
          }
          if (j != blocks_y - 1)
          {
            Grid &R = ring_buffers[i*blocks_y*blocks_z + (j+1)*blocks_z + k][cur_grid];
            ...
            for (int x = 0; x < S.xn; x++)
              for (int z = 0; z < S.zn; z++)
                R.data[x*R.yn*R.zn + (0)*R.zn + z] = S.data[x*S.yn*S.zn + (S.yn-2)*S.zn + z];          
          }

          if (k != 0)
          {
            Grid &R = ring_buffers[i*blocks_y*blocks_z + j*blocks_z + k-1][cur_grid];
            //printf("(%d %d %d) %d %d   %d %d   %d %d+%d\n", i,j,k, S.xn, R.xn, S.yn, R.yn, S.zn, R.zn, R.z0);
            assert(S.xn == R.xn && S.yn == R.yn);
            for (int x = 0; x < S.xn; x++)
              for (int y = 0; y < S.yn; y++)
                R.data[x*R.yn*R.zn + y*R.zn + (R.zn-1)] = S.data[x*S.yn*S.zn + y*S.zn + (1)];
          }
          if (k != blocks_z - 1)
          {
            Grid &R = ring_buffers[i*blocks_y*blocks_z + j*blocks_z + k+1][cur_grid];
            ...
            for (int x = 0; x < S.xn; x++)
              for (int y = 0; y < S.yn; y++)
                R.data[x*R.yn*R.zn + y*R.zn + (0)] = S.data[x*S.yn*S.zn + y*S.zn + (S.zn-2)];          
          }
        }      
      }
    }
...
int main(int argc, char **argv)
{
  ...
  {
    fill_reference_grid(ring_buffers[block_n][0], 0*tau);
    fill_reference_grid(ring_buffers[block_n][1], 1*tau);
    Grid &P1 = ring_buffers[block_n][0];
    Grid &P_res = ring_buffers[block_n][1];
    ...
    ring_buffers[block_n][2].clear_and_fill_borders(0);
  }
  transfer(blocks_x, blocks_y, blocks_z, 1, ring_buffers);
  /*
  {
    Grid &P1 = ring[0];
    double c = (tau*tau*a_squared()/(h*h));
  ...
  */
  ...
  for (int step = 2; step<steps; step++)
  {
    ...
    //transfer
    t1 = clock();
    transfer(blocks_x, blocks_y, blocks_z, cur_grid, ring_buffers);
    t2 = clock();
    solve_ms += 1000*(double)(t2 - t1) / (CLOCKS_PER_SEC * THREADS_NB);
